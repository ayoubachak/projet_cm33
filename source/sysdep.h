...
int pcm_vol(real *vol);						// set the inut and output volume levels
real pcm_freq(void);						// get CODEC sampling frequency
int pcm_play(real *data, int n_ch, int n);	// data, nb channels, nb samples
int pcm_rec(real *data, int n);				// return data, nb samples
int pcm_loop();								// loopback audio signal

void captaudio_cb(cplx*, int, int);
void filter_data(int, real*, int);
